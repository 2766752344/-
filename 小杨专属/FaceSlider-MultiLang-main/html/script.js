...
const config = {
    ...
    states: {
        ...
        happy: {
            ...
            ui: {
                btnHappyText: '再想小董一万次',
                titleText: '小董今天也超级无敌想小杨宝宝',
                subtitleText: '❤️❤️❤️'
            }
        },
        unhappy: {
...
function transitionToState(stateType, hideButton = null) {
    ...
    if (targetState.ui) {
        ...
        if (subtitleText) {
            if (state.customSubtitle) {
                subtitle.innerHTML = state.customSubtitle;
            } else {
...
btnUnhappy.addEventListener('click', () => {
    if (state.animationId) {
        ...
        if (state.rejectCount === 1) {
            ...
            transitionToState('unhappy');
        } else if (state.rejectCount === 2) {
            btnUnhappy.style.position = 'absolute';
            btnUnhappy.style.left = `${Math.random() * 80}%`;
            btnUnhappy.style.top = `${Math.random() * 80}%`;
            state.target.happiness = Math.max(0.1, state.target.happiness - 0.1);
            btnHappy.style.transform = `scale(${1 + state.rejectCount * 0.1})`;
            state.customSubtitle = '再V50加肯德基';
            subtitle.innerHTML = state.customSubtitle;
        } else if (state.rejectCount >= config.maxUnhappyCount) {
            stopAnimation();
